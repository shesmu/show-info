...
module.exports.show_info = function(search, callback){
	...
	request(url, function (error, response, html){
		...
		$('.result.show').eq(0).filter(function(){
			var data = $(this),
				...
				getDescription(img, show_name, air_day, episode_guide, description, callback);
		})
	})
	function getDescription(img, show_name, air_day, episode_guide, description, callback) {
		request(description, function (error, response, html){
			...
			$('.description').eq(0).filter(function (){
				var data = $(this),
					...
					var show = { img:img, show_name:show_name, air_day:air_day, episode_guide:episode_guide.click(description), description: finalDescription};
					callback(show);
			})
		})
	}
...
function test (input){
	console.log(input.episode_guide);
}


show_info('rick and morty', test);
